...
public interface OrderRepository extends JpaRepository<Order,Long>{
    ...
    @Query(value = "SELECT * FROM order o JOIN cartitem c ON o.item_fk = c.id_pk JOIN account a ON c.consumer_fk = a.id_pk WHERE username=:username",nativeQuery = true)
   ...
   // @Query("select o from order a where o.item_fk.consumer_fk.city=:city")
    @Query(value = "SELECT * FROM order o JOIN cartitem c ON o.item_fk = c.id_pk JOIN account a ON c.consumer_fk = a.id_pk WHERE city=:city",nativeQuery = true)
    public List<Order> findAllByCity(@Param("city")String city);
    
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query(value = "UPDATE order SET status = :status WHERE id_pk = :id", nativeQuery = true)
